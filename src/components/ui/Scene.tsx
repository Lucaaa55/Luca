...
function Banana({ index, z, speed }: { index: number, z: number, speed: number }) {
    const ref = useRef<THREE.Object3D>(null)
    
    const { viewport, camera } = useThree()
    const { width, height } = viewport.getCurrentViewport(camera, [0, 0, -z])
    const { nodes, materials } = useGLTF('/assets/banana.glb')    
    const [data] = useState({
    ...
    return (
        <Canvas flat gl={{ antialias: false }} dpr={[1, 1.5]} camera={{ position: [0, 0, 10], fov: 20, near: 0.01, far: depth + 15 }}>
            ...
            <EffectComposer enableNormalPass={false} multisampling={0}>
                <DepthOfField target={[0, 0, 60]} focalLength={0.7} bokehScale={2} height={700} />
                <ToneMapping />
            </EffectComposer>
        </Canvas>
    )
